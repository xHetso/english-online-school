<template>
  <div class="p-4">
    <form @submit.prevent="submitEdit(props.authorId)" class="space-y-4 text-white bg-purple-100 p-4 rounded-lg">
      <div>
  ...
  async function uploadFile(event) {
    ...
    try {
      const response = await fetch('https://backend-english-school.onrender.com/api/files?folder=authors', {
        method: 'POST',
        body: formData
      });
